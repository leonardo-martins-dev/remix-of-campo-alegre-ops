...
/**
 * Thin proxy: triggers wise-sync-service compra sync.
...
Deno.serve(async (req) => {
  ...
  if (!base || !secret) {
    return new Response(
      JSON.stringify({
        ...
        rows: [],
        message:
          "WISE_SYNC_URL / WISE_SYNC_CRON_SECRET não configurados. O pull diário roda no wise-sync-service (cron 06:00). Use importação por arquivo.",
      }),
  ...
  try {
    const res = await fetch(`${base.replace(/\/$/, "")}/v1/sync/compra`, {
      method: "POST",
      headers: {
    ...
    return new Response(
      JSON.stringify({
        ...
        message: body.empty_window
          ...
          : body.ok
            ? `Sync compra ok · ${body.pedidos ?? 0} pedidos`
