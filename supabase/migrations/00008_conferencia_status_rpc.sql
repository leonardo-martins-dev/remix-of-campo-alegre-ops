-- RPC com cast explícito + trigger simplificado (sem UPDATE em pedidos_recebimento)
-- Corrige erro: column "status" is of type public.status_pedido but expression is of type text
...
BEGIN
  IF NEW.status = 'finalizada'::public.status_conferencia
    ...
    NEW.finalizada_em = COALESCE(NEW.finalizada_em, now());
  END IF;
  RETURN NEW;
END;
$$;
...
REVOKE ALL ON FUNCTION public.update_conferencia_status(UUID, UUID, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.update_conferencia_status(UUID, UUID, TEXT) TO authenticated;
