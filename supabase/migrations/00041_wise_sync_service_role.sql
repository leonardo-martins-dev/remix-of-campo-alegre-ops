-- 00041: allow service_role to call importar_pedidos_wise (cron wise-sync-service)
-- Actor: auth.uid() when JWT user; else first admin profile when role=service_role.
...
GRANT EXECUTE ON FUNCTION public.importar_pedidos_wise(TEXT, TEXT, TEXT, JSONB, JSONB) TO service_role;

COMMENT ON FUNCTION public.importar_pedidos_wise IS
  'Import Wise pedidos; callable by authenticated users and service_role (wise-sync-service cron).';
